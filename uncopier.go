...
func headersByRequestURI() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if tls == "https" {
			...
			c.Header("Content-Security-Policy", "unsafe-inline; base-uri 'self'; default-src 'self' https://"+serverHost+"; font-src 'self'; img-src 'self' data: http://www.w3.org; script-src 'self' https://www.paypal.com/; frame-src 'self'; style-src 'self' https://fonts.googleapis.com https://cdn.jsdelivr.net;")
