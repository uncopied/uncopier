...
func headersByRequestURI() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if tls == "https" {
			...
			c.Header("Content-Security-Policy", "base-uri 'self'; default-src 'self' https://"+serverHost+"; font-src 'self' data: https://cdn.jsdelivr.net; connect-src 'self' https://www.paypal.com/ ; img-src 'self' data: http://www.w3.org https://t.paypal.com/; script-src 'self' 'unsafe-inline' https://www.paypal.com/; frame-src 'self' https://www.paypal.com/; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdn.jsdelivr.net;")
